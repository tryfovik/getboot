...
| `getboot-coordination` | 分布式协调底座 | 需要 Redisson，或者准备接锁、限流、Webhook 时 | `getboot-lock` / `getboot-limiter` / `getboot-webhook` | `getboot.coordination.redisson.*` | `redisson` | [getboot-coordination/README.md](../getboot-coordination/README.md) |
...
| `getboot-limiter` | 分布式限流 | 需要声明式限流或统一限流规则时 | `getboot-coordination` | `getboot.limiter.*` + `getboot.coordination.redisson.*` | `slidingwindow.redisson` / `tokenbucket.redisson` / `leakybucket.redisson` | [getboot-limiter/README.md](../getboot-limiter/README.md) |
| `getboot-lock` | 分布式锁 | 需要声明式锁注解和默认锁键解析策略时 | `getboot-coordination` | `getboot.lock.*` + `getboot.coordination.redisson.*` | `redis.redisson` | [getboot-lock/README.md](../getboot-lock/README.md) |
