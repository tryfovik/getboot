...
| `getboot-mail` | 邮件发送能力 | 需要统一 SMTP 邮件发送、模板变量替换、附件与默认发件人时 | 无强制配套模块；告警或通知场景可按需搭配 `getboot-observability` | `getboot.mail.*` | `smtp` | [getboot-mail/README.md](../getboot-mail/README.md) |
| `getboot-ai` | AI 能力 | 需要统一 Chat、Embedding、Rerank 和提示词模板渲染时 | `getboot-search` / `getboot-storage` 可选，用于检索增强或文件上下文场景 | `getboot.ai.*` | `openai` / `embedding-rerank` | [getboot-ai/README.md](../getboot-ai/README.md) |
...
| `getboot-mq` | 消息能力 | 需要 RocketMQ / Kafka / MQTT 统一生产入口、Trace 透传，或 RocketMQ 事务消息路由时 | `getboot-observability` | `getboot.mq.*` | `rocketmq` / `kafka` / `mqtt` | [getboot-mq/README.md](../getboot-mq/README.md) |
| `getboot-websocket` | WebSocket 长连接 | 需要统一 WebSocket 会话注册、按用户/会话推送和连接生命周期监听时 | `getboot-support` 可选，用于 Trace 上下文恢复 | `getboot.websocket.*` | `jakarta.websocket` | [getboot-websocket/README.md](../getboot-websocket/README.md) |
