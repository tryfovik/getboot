...
- [ ] `getboot-exception`：当前 `2` 个测试；补错误码边界与异常构造分支
- [ ] `getboot-web`：当前 `2` 个测试；补统一响应边界与分页请求异常分支
- [ ] `getboot-database`：当前 `1` 个测试；补数据源别名桥接或自动配置边界
...
- [ ] `getboot-governance`：当前 `2` 个测试；补配置绑定与自动配置边界
- [ ] `getboot-webhook`：当前 `1` 个测试；补自动配置、验签/限流/幂等组合边界
