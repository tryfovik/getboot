...
`dt-starter-suite` 是一组面向 Spring Boot 3 的基础能力模块。它不是再造一套大而全的后台脚手架，而是把业务服务里最常重复、最容易散落、又最难持续维护的一批基础能力收成可复用、可按需引入的 starter。
...
| 分布式限流 | `getboot-coordination` + `getboot-limiter` | 当前已支持滑动窗口实现 |
