...
- 每起一个新服务，都要重新拼父 `pom`，重新核 Spring Boot、Spring Cloud、Redis、Dubbo、RocketMQ、Kafka、XXL-JOB、支付 SDK 的版本组合
- 每个项目都自己补统一响应、统一异常、`traceId`、默认 `RedisTemplate`、分布式锁、限流、Webhook 验签、请求头透传
- 同样是 MQ、RPC、缓存、支付接入，不同团队能长出完全不同的配置前缀、消息模型、异常语义和默认约定
...
- 亏在排障成本：链路一长，`traceId`、Header、异常语义不统一，问题定位会成倍变慢
- 亏在升级成本：同一个依赖升级动作，可能要在多个业务仓库里分别验证、分别踩坑
